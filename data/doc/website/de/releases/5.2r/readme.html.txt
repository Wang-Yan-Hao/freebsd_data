...
>>>>>>> b1bb5fd9 (Processing txt files in data/doc)
  ...
  Dieses Dokument enthält eine kurze Einführung zu FreeBSD 5.2-RELEASE.
  Es enthält einige Hinweise, wie Sie FreeBSD beziehen können; wie Sie
...
5.2-RELEASE und repräsentiert den aktuellen Zustand im Entwicklungszweig
5-CURRENT.
...
AMD, Cyrix oder NexGen ``x86''-basierte PCs (i386™), NEC PC-9801/9821
und kompatible Systeme (pc98), DEC/Compaq/HP Alpha Systeme (alpha) und
UltraSPARC Systeme (Sparc64®). Versionen für die Plattformen IA64
(ia64), PowerPC® (PowerPC) und AMD ``Hammer'' (amd64) sind ebenfalls in
der Entwicklung. FreeBSD unterstützt viele verschiedene Geräte und
Umgebungen und kann für alle Anwendungen von der Software-Entwicklung
über Spiele bis zur Anbietung von Diensten im Internet genutzt werden.
...
den Kernel und alle Programme des Basissystems. Wenn Sie die ``source''
...
können. Die über 10,000 Ports (vom Editor über Programmiersprachen bis
zu graphischen Anwendungen) machen FreeBSD zu einer leistungsstarken und
...
Diese release von FreeBSD ist für alle Arten von Benutzern gedacht. Sie
wurde ausführlich geprüft und getestet, um die größtmögliche
Zuverlässigkeit und Betriebssicherheit garantieren zu köönnen.
...
Erzeugung einer (bootfähigen) CDROM der FreeBSD-Release benötigen.
...
Mailingliste FreeBSD-CURRENT lesen. Nur so können Sie die teilweise
lebensnotwendigen Informationen über die aktuellen Entwicklungen und
Änderungen erhalten, die Sie für den Betrieb des Systems benötigen.
...
Mailingliste FreeBSD announcements konzentrieren.
...
Info Page. Dort erhalten Sie weitergehende Informationen, wie Sie die
diversen Mailinglisten abonnieren können, wie Sie auf die Archive
zugreifen können, usw. Es gibt viele Mailinglisten, die nur mit einem
ganz bestimmten Thema befassen und hier nicht aufgeführt sind.
Weitergehende Informationen finden Sie auf den Mailman Seiten und im
Bereich Mailinglisten der FreeBSD Webseite.
...
send-pr(1) benutzen. Wenn Sie Ihren ``Problem Report'' (PRs) auf dieser
Art und Weise absetzen, wird er automatisch archiviert und
weiterverfolgt. Die Entwickler von FreeBSD bemühen sich, so schnell wie
möglich auf alle gemeldeten Fehler zu reagieren. Eine Übersicht über
alle offenen PRs ist auf dem Webserver von FreeBSD verfügbar. Diese
Liste ist sehr nützlich, wenn Sie wissen wollen, welche möglichen
...
Sie finden weitergehende über diese release Distribution in anderen
...
-   EARLY.TXT: Spezielle Hinweise für die ersten Anwender von FreeBSD
    5.2-RELEASE. Wenn dies Ihre erste Begegnung mit FreeBSD 5-CURRENT
...
-   RELNOTES.TXT: Die begleitenden Informationen zu dieser Release
    enthalten die Neuerungen in dieser Version von FreeBSD (5.2-RELEASE)
    und die Änderungen seit der letzten Version (FreeBSD 5.1-RELEASE).
...
Zeit alpha, i386, ia64, pc98 und Sparc64) finden Sie diese Dokumente
während der Installation normalerweise unter dem Menüpunkt
Dokumentation. Um nach der Installation des Systems dieses Menü
zugreifen zu können, müssen Sie das Programm sysinstall(8) erneut
aufrufen.
...
aus der ganzen Welt bedanken, ohne die diese release niemals möglich
gewesen wäre.
